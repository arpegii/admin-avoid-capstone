...
export const exportReportAsWorkbook = async ({
  ...
  if (reportAnalytics?.summaryRows?.length) {
    ...
    const summaryTitleCell = worksheet.getCell(rowPointer, 1);
    summaryTitleCell.value = "Analytics Summary";
    summaryTitleCell.font = { bold: true, size: 12, color: { argb: "FF0F172A" } };
    summaryTitleCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE2E8F0" } };
    summaryTitleCell.alignment = { horizontal: "left", vertical: "middle" };
    applyThinBorder(summaryTitleCell);
    rowPointer += 1;

    reportAnalytics.summaryRows.forEach(([label, value]) => {
      worksheet.getCell(rowPointer, 1).value = `${label}:`;
      worksheet.getCell(rowPointer, 2).value = String(value ?? "");
      worksheet.getCell(rowPointer, 1).font = { bold: true, color: { argb: "FF1E293B" } };
      worksheet.getCell(rowPointer, 2).font = { color: { argb: "FF334155" } };
      worksheet.getCell(rowPointer, 1).fill = META_LABEL_FILL;
      applyThinBorder(worksheet.getCell(rowPointer, 1));
      applyThinBorder(worksheet.getCell(rowPointer, 2));
      rowPointer += 1;
    });
    rowPointer += 1;
  }
  ...
  if ((reportChartImages || []).length) {
    for (const chartImage of reportChartImages) {
      worksheet.mergeCells(rowPointer, 1, rowPointer, totalColumns);
      const chartTitleCell = worksheet.getCell(rowPointer, 1);
      chartTitleCell.value = chartImage.title || "Analytics Chart";
      chartTitleCell.font = { bold: true, color: { argb: "FF0F172A" } };
      chartTitleCell.alignment = { horizontal: "left", vertical: "middle" };
      applyThinBorder(chartTitleCell);
      rowPointer += 1;
      ...
      worksheet.addImage(imageId, {
        tl: { col: 0, row: rowPointer - 1 },
        ext: { width: 760, height: 210 },
      });
      rowPointer += 12;
    }
    rowPointer += 1;
